...
ciagent init --generate               # Guided interview: auto-scan, generate ciagent_spec.yaml
...
ciagent validate ciagent_spec.yaml    # Validate spec against schema (no execution)
...
ciagent record <test_name> -s ciagent.yaml  # DEPRECATED v1 path (single golden trace)
...
# Assertions (used in ciagent.yaml, evaluated by runner)
...
### YAML-Based Test Configuration (ciagent.yaml, legacy v1)

> The v1 `tests:` format below is deprecated and will be removed in 0.9.0.
> New projects should use `ciagent_spec.yaml` (the `queries:` format) with
...
### Assertion Types (for ciagent.yaml)
...
ciagent record                # all queries in ciagent_spec.yaml
...
# Legacy v1 flow (ciagent.yaml, removed in 0.9.0):
#   ciagent record test_billing_routing -s ciagent.yaml -o golden/billing.json
#   ciagent run
...
```
my-agent-project/
├── ciagent.yaml              # Test suite configuration
├── tests/
...
└── .github/
    └── workflows/
        └── ciagent.yml       # Generated by `ciagent init`
```
...
add_trace_processor(CIAgentTraceProcessor())
```
...
- `"No agent import path provided in test suite."` — Set `agent: myapp.agent:run_agent` in ciagent.yaml
...
```bash
ciagent init         # Generates .github/workflows/ciagent.yml
